...
from proj.environment import Environment
from proj.agent import Agent
from proj.networks.networks_keras import Network, DNN, LSTMNetwork, CNN
from proj.visualizer import Visualizer
...
class ReinforcementLearner:
    ...
    #            min_trading_price=100000, max_trading_price=10000000,
        ...
        # 강화학습 환경 생성(환경은 차트 데이터를 순서대로 읽으면서 주가, 거래량등의 데이터 제공)
        ...
        # 학습 데이터
        self.training_data = training_data          # 학습을 위한 전처리된 데이터
        self.sample = None
        ...
        # 신경망 설정
        self.net = net                  # 신경망 종류(DNN, LSTM, CNN등)
        self.num_steps = num_steps      # LSTM, CNN 신경망에서 사용하는 샘플 묶음 크기
        self.lr = lr                    # 학습속도
    ...
    def init_value_network(self, shared_network=None, activation='linear', loss='mse'):
        if self.net == 'dnn':
            ...
            self.value_network = CNN(
                input_dim=self.num_features, 
                output_dim=self.agent.NUM_ACTIONS, 
                lr=self.lr, num_stepㄷs=self.num_steps,
                shared_network=shared_network,
                activation=activation, loss=loss)

        # reuse model 이 True이고 value_network_path 경로에 파일이 존재하면 신경망 파일 불러옴
        if self.reuse_models and os.path.exists(self.value_network_path):
            self.value_network.load_model(model_path=self.value_network_path)

    # 정책 신경망 생성(가치 신경망과 유사하나 차이는 활성화 함수를 sigmoide로 사용)
    ...
    def reset(self):
        ...
        self.exploration_cnt = 0    # 무작위 투자를 수행한 횟수 저장
        self.batch_size = 0
